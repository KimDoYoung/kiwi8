...
class MenuResponse(BaseModel):
    ...
    parent_id: int | None = Field(None, description="부모 메뉴 ID")
    level: int = Field(..., description="메뉴 레벨 (1:대, 2:중, 3:소)")
    screen_no: str | None = Field(None, description="화면 번호 (말단 메뉴)")
    title: str = Field(..., description="메뉴 표시 명칭")
    url: str | None = Field(None, description="이동 경로")
    component: str | None = Field(None, description="React 컴포넌트명")
    icon: str | None = Field(None, description="아이콘 이름 (Lucide kebab-case)")
