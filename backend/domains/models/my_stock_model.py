...
class MyStockUpdate(BaseModel):
    ...
    """
    stk_nm: str | None = Field(None, description="종목명", max_length=100)
...
class MyStockFilter(BaseModel):
    ...
    - 여러 조건을 AND로 결합하여 검색
    """
    is_hold: int | None = Field(None, description="보유여부 필터 (0: 미보유, 1: 보유)")
    is_watch: int | None = Field(None, description="관심여부 필터 (0: 비관심, 1: 관심)")
    sector: str | None = Field(None, description="특정 섹터 필터", example="반도체")
