...
class SchedulerRunHistory:
    ...
    finished_at: str | None = None                          # 완료 시각
    status: str | None = None                               # 상태: 'success' | 'error' | 'cancelled' | 'timeout'
    message: str | None = None                              # 실행 결과 메시지
...
class SchedulerLock:
    ...
    holder: str | None = None                               # 락 소유자 (hostname/pid)
    acquired_at: str | None = None                          # 획득 시각
...
class SchedulerJobUpdate(BaseModel):
    """스케줄러 작업 수정 요청 모델"""
    name: str | None = Field(None, description="작업명", max_length=255)
    func_name: str | None = Field(None, description="실행할 함수 키", max_length=255)
    schedule_type: Literal["interval", "cron", "once"] | None = Field(None, description="스케줄 타입")
    schedule_expr: str | None = Field(None, description="스케줄 표현식")
    timezone: str | None = Field(None, description="타임존", max_length=50)
    enabled: int | None = Field(None, description="활성화 여부", ge=0, le=1)
    max_conc: int | None = Field(None, description="최대 동시 실행 수", ge=1)
    overlap_policy: Literal["skip", "queue", "cancel"] | None = Field(None, description="중복 실행 정책")
    timeout_sec: int | None = Field(None, description="타임아웃 (초)", ge=0)
    retry_max: int | None = Field(None, description="최대 재시도 횟수", ge=0)
    retry_backoff: float | None = Field(None, description="지수 백오프 배수", ge=1.0)
    jitter_sec: int | None = Field(None, description="지터 (초)", ge=0)
...
class SchedulerJobResponse(BaseModel):
    ...
    next_run_at: str | None = Field(None, description="다음 실행 시각")
    last_run_at: str | None = Field(None, description="마지막 실행 시각")
...
class SchedulerRunHistoryCreate(BaseModel):
    ...
    finished_at: str | None = Field(None, description="완료 시각")
    status: Literal["success", "error", "cancelled", "timeout"] | None = Field(None, description="실행 상태")
    message: str | None = Field(None, description="실행 결과 메시지")
...
class SchedulerRunHistoryResponse(BaseModel):
    ...
    started_at: str = Field(..., description="시작 시각")
    finished_at: str | None = Field(None, description="완료 시각")
    status: str | None = Field(None, description="실행 상태")
    message: str | None = Field(None, description="실행 결과 메시지")
...
class SchedulerJobFilter(BaseModel):
    """스케줄러 작업 필터링 모델"""
    enabled: int | None = Field(None, description="활성화 여부 필터 (0: 비활성, 1: 활성)")
    schedule_type: Literal["interval", "cron", "once"] | None = Field(None, description="스케줄 타입 필터")
    func_name_like: str | None = Field(None, description="함수명 부분 검색")
    name_like: str | None = Field(None, description="작업명 부분 검색")

class SchedulerRunHistoryFilter(BaseModel):
    """스케줄러 실행 이력 필터링 모델"""
    job_name: str | None = Field(None, description="특정 작업명 필터")
    status: Literal["success", "error", "cancelled", "timeout"] | None = Field(None, description="실행 상태 필터")
    started_after: str | None = Field(None, description="시작 시각 이후 필터 (ISO8601)")
    started_before: str | None = Field(None, description="시작 시각 이전 필터 (ISO8601)")
