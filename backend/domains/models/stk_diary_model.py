...
class StkDiary:
    ...
    stk_cd: str | None = None               # 종목코드 (선택사항, 전체 시장 일지도 가능)
...
class StkDiaryCreate(BaseModel):
    ...
    stk_cd: str | None = Field(None, description="종목코드 (선택사항)", example="005930")
    note: str = Field(..., description="일지 내용", example="삼성전자 실적 분석...")

class StkDiaryUpdate(BaseModel):
    """주식 일지 수정 요청 모델"""
    ymd: str | None = Field(None, description="날짜 (YYYYMMDD 형식)")
    stk_cd: str | None = Field(None, description="종목코드")
    note: str | None = Field(None, description="일지 내용")
...
class StkDiaryFilter(BaseModel):
    """주식 일지 필터링 모델"""
    ymd_from: str | None = Field(None, description="시작 날짜 (YYYYMMDD)")
    ymd_to: str | None = Field(None, description="종료 날짜 (YYYYMMDD)")
    stk_cd: str | None = Field(None, description="특정 종목코드 필터")
    note_like: str | None = Field(None, description="일지 내용 검색어")
