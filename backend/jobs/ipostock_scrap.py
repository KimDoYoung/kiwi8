...
def collect_ipo() -> tuple[int, int, int]:
    """ipostock.co.kr 스크래핑 → ipo_data upsert. (inserted, updated, skipped) 반환."""
    scraper = Scraper()
    data_list = []
    code_list = []
    ...
    for code in code_list:
        ...
        try:
            ...
            from backend.core.logger import get_logger
            get_logger(__name__).warning(f'[ipo_scrap] 스크래핑 실패 ({code}): {e}')

    repo = IpoRepository()
    return repo.upsert(data_list)
