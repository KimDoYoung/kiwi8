...
def df_change(df):
    ...
    # [공통] 실수 변환 헬퍼 함수: %, 콤마 제거 및 NaN 대응
    def to_safe_float(series):
        if series is None: return 0.0
        return pd.to_numeric(series.astype(str).str.replace('%', '').str.replace(',', ''), errors='coerce').fillna(0.0).astype(float)
    ...
    if '버핏초이스' in df.columns:
        # '위' 글자 제거 및 공백 제거
        temp_buffet = df['버핏초이스'].astype(str).str.replace(' ', '').str.replace('위', '')
        # 숫자로 변환 시도, 실패(NaN)하면 99999로 채움
        df['버핏초이스'] = pd.to_numeric(temp_buffet, errors='coerce').fillna(99999).astype(int)        
    ...
    if '현재가격' in df.columns:
        df[['현재가', '등락가']] = df['현재가격'].str.extract(cost_range_pattern)
        # 수정: fillna를 먼저 하고, replace 후 다시 한번 결측치를 0으로 채움
        df['현재가'] = df['현재가'].str.replace(',', '').fillna('0')
        df['등락가'] = df['등락가'].str.replace(',', '').fillna('0')
        # 데이터가 아예 없는 경우(빈 문자열 등)를 대비해 to_numeric 사용
        df['현재가'] = pd.to_numeric(df['현재가']).fillna(0).astype(int)
        df['등락가'] = pd.to_numeric(df['등락가']).fillna(0).astype(int)
        df.drop(columns=['현재가격'], inplace=True)

    if '52주 최고최저' in df.columns:
        df[['52주최고', '52주최저']] = df['52주 최고최저'].str.extract(cost_range_pattern)
        # 수정: 안전한 변환
        df['52주최고'] = pd.to_numeric(df['52주최고'].str.replace(',', '')).fillna(0).astype(int)
        df['52주최저'] = pd.to_numeric(df['52주최저'].str.replace(',', '')).fillna(0).astype(int)
