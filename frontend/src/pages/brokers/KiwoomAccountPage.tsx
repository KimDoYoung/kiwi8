...
export default function KiwoomAccountPage() {
    ...
    const stocks = useMemo(() => {
        ...
        if (filterCodes.length > 0) {
            list = list.filter(s => filterCodes.includes(String(s.종목코드)))
        }
    ...
    const colDefs = useMemo<ColDef[]>(() => {
        const allCols: ColDef[] = [
            ...
            {
                field: '보유수량', headerName: '수량', width: 80, type: 'numericColumn', context: { simple: true },
                valueFormatter: ({ value, data }) => (data?._isSummary && toNum(value) === 0) ? '' : fmt(toNum(value)),
                comparator: numComparator,
            },
            {
            ...
            {
                field: '손익금액', headerName: '손익금액', width: 120, type: 'numericColumn', context: { simple: true },
                cellRenderer: (p: CustomCellRendererProps) => (p.data?._isSummary && toNum(p.value) === 0) ? '' : <ProfitCell {...p} />,
                comparator: numComparator,
            },
            {
                field: '손익율', headerName: '손익율(%)', width: 95, type: 'numericColumn', context: { simple: true },
