  ...
  return (
    <div className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
      <div className="px-3 py-2 bg-gray-50 border-b border-gray-200 flex items-center gap-2">
        ...
        {badge && (
          <span className="ml-auto text-[11px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-semibold">
            {badge}
          </span>
        )}
      </div>
...
function InfoRow({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div className="flex gap-2 py-0.5 border-b border-gray-50 last:border-0">
      <span className="text-gray-400 w-20 shrink-0 text-[12px]">{label}</span>
      <span className="text-gray-800 font-medium text-[12px] break-all">{value || '-'}</span>
    </div>
  )
}
...
function StkInfoSection({ stk_cd }: { stk_cd: string }) {
  ...
  if (!data) return (
    <div className="text-gray-400 text-[12px] py-4 text-center">stk_info 데이터 없음 (fill_stk_info 실행 필요)</div>
  )
    ...
    <SectionCard title="기본 정보 (stk_info)" icon={Building2}>
      ...
      {data.homepage && (
        <a
          ...
          className="mt-2 inline-flex items-center gap-1 text-[12px] text-blue-600 hover:underline"
        >
...
function JudalSection({ stk_cd }: { stk_cd: string }) {
  ...
  if (!themes.length) return (
    <div className="text-gray-400 text-[12px] py-4 text-center">주달 데이터 없음 (judal_themes 수집 필요)</div>
  )
  ...
  return (
    <SectionCard title="주달 분석 (judal_themes)" icon={BarChart3} badge={`${themes.length}개 테마`}>
      <div className="grid grid-cols-2 gap-x-6 mb-3">
        <div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">현재가</span>
            <span className="text-gray-800 font-bold text-[12px]">{fmt(t.current_price)}원</span>
          </div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">전일비</span>
            <RateBadge value={t.yesterday_ratio} />
          </div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">3일합산</span>
            <RateBadge value={t.three_day_sum} />
          </div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">기대수익률</span>
            <RateBadge value={t.expected_return} />
          </div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">버핏초이스</span>
            <span className="text-gray-800 font-medium text-[12px]">{t.buffett_choice || '-'}</span>
          </div>
        </div>
        <div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">PER</span>
            <span className="text-gray-800 font-medium text-[12px]">{t.per?.toFixed(2) || '-'}배</span>
          </div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">PBR</span>
            <span className="text-gray-800 font-medium text-[12px]">{t.pbr?.toFixed(2) || '-'}배</span>
          </div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">EPS</span>
            <span className="text-gray-800 font-medium text-[12px]">{fmt(t.eps)}원</span>
          </div>
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">52주 고/저</span>
          ...
          <div className="flex gap-2 py-0.5 border-b border-gray-50">
            <span className="text-gray-400 w-20 shrink-0 text-[12px]">52주소외지수</span>
            <span className="text-gray-800 font-medium text-[12px]">{t.alienation_index_52w ?? '-'}</span>
          </div>
        </div>
      </div>

      <div>
        <div className="text-[12px] text-gray-500 font-semibold mb-1">관련 테마 ({themeNames.length}개)</div>
        <div className="flex flex-wrap gap-1">
          {themeNames.map(name => (
            <span key={name} className="px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded text-[11px] font-medium">
              {name}
            </span>
          ))}
        </div>
      ...
      {t.updated_at && (
        <div className="text-[11px] text-gray-400 mt-2 text-right">수집: {t.updated_at}</div>
      )}
    </SectionCard>
  )
}
...
function OptionsSection({ stk_cd }: { stk_cd: string }) {
  ...
  return (
    ...
    >
      ...
      {!optList.length ? (
        <div className="text-gray-400 text-[12px] py-4 text-center">
            ...
            return (
              <div key={opt.id} className="border border-gray-100 rounded overflow-hidden">
                ...
                >
                  ...
                  <span className="font-semibold text-blue-700 text-[12px] shrink-0">{fmtDate(opt.date)}</span>
                  <span className="text-gray-500 text-[11px] ml-auto shrink-0">{lines.length}건</span>
                </button>
                {isOpen && (
                ...
                {!isOpen && (
                  <div className="px-3 py-1.5 bg-white border-t border-gray-50">
                    <pre className="text-[11px] text-gray-400 leading-relaxed whitespace-pre-wrap font-sans line-clamp-3">
                      {preview}
                    </pre>
                  </div>
                )}
...
export default function StockEvaluationPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100 text-[13px]">
      {/* Header */}
      <div className="h-11 px-4 border-b border-gray-300 bg-gray-50 flex items-center gap-4 shrink-0 shadow-sm z-10">
        ...
        {stk_cd && (
          <div className="ml-2 flex items-center gap-1.5">
            ...
            <span className="font-mono text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-200 text-[12px] font-bold">
              {stk_cd}
            </span>
          </div>
        )}
      ...
      {!stk_cd ? (
        <div className="flex-1 flex flex-col items-center justify-center text-gray-400 bg-white m-3 rounded-xl border border-dashed border-gray-300">
          ...
          <h2 className="text-[19px] font-bold text-gray-700 mb-1">종목을 선택해 주세요</h2>
          <p className="text-[15px]">상단 검색 바에 종목명 또는 코드를 입력하세요.</p>
        </div>
      ) : (
