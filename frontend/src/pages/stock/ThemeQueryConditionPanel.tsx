  ...
  return (
    <div className="w-80 p-4 space-y-4 bg-white shadow-xl rounded-lg border">
      ...
      <div className="space-y-1">
        <FilterRow label="현재가" field="current_price" step={100} />
        <FilterRow label="시가총액" field="market_cap" step={10} unit="억" />
        <FilterRow label="전일비" field="yesterday_ratio" step={0.1} unit="%" />
        <FilterRow label="3일합산" field="three_day_sum" step={0.1} unit="%" />
        <FilterRow label="PER" field="per" step={0.1} />
        <FilterRow label="PBR" field="pbr" step={0.1} />
      </div>
